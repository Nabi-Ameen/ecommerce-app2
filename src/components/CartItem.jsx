      ...
      <div className='w-full min-h-[150px] flex items-center gap-x-4'>
        ...
        <div className='w-full flex flex-col'>
          <div className='flex justify-between mb-2'>
            <Link to={`/product/${id}`} className="text-xs uppercase font-medium max-w-[240px] text-primary hover:underline">{title}
            </Link>
            <div onClick={() => removeCartItem(id)} className='text-xl  cursor-pointer'>
              <IoMdClose className='text-gray-600 hover:text-blue-600 transition-all' />
            </div>
          </div>
          <div>
            <div className='flex gap-x-2 h-[36px] text-sm'>
              ...
              <div className='flex-1 flex items-center justify-around'>$ {price}
              </div>
